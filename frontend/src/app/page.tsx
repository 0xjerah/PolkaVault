...
function VaultStats() {
  ...
  const items = [
    { icon: TrendingUp, label: "Exchange Rate", value: fmtRate(rate),   unit: "PAS / stDOT", color: "text-pink-400",    glow: "shadow-pink-500/10"    },
    { icon: Lock,       label: "Total Staked",  value: fmt(staked),     unit: "PAS bonded",  color: "text-emerald-400", glow: "shadow-emerald-500/10" },
    { icon: Layers,     label: "stDOT Supply",  value: fmt(supply),     unit: "tokens",      color: "text-blue-400",    glow: "shadow-blue-500/10"    },
    { icon: Zap,        label: "Est. APY",       value: "~12–15%",       unit: "native yield", color: "text-yellow-400",  glow: "shadow-yellow-500/10"  },
  ] as const;

  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-10 max-w-3xl mx-auto w-full">
      {items.map(({ icon: Icon, label, value, unit, color, glow }) => (
        <div
          key={label}
          className={clsx(
            "rounded-2xl bg-[#0d0d18] border border-white/[0.08] p-4 text-center shadow-lg hover:border-white/[0.14] transition-all",
            glow
          )}
        >
          <Icon className={clsx("w-4 h-4 mx-auto mb-2", color)} />
          <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">{label}</p>
          <p className={clsx("font-black text-base leading-none", color)}>{value}</p>
      ...
      <main className="min-h-screen">
        ...
        <footer className="border-t border-white/[0.05] py-8 text-center">
          <p className="text-xs text-gray-700">
            ...
            <a href="https://blockscout-testnet.polkadot.io/address/0x3e9eF811ddF3078559178C57d6cD97f45DbD6220"
